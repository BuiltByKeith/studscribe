...
// Sidebar dimensions are theme values, not component constants -- they live as
// --sidebar-width, --sidebar-width-icon, and --sidebar-width-mobile in
// resources/css/globals.css. Because they are declared on :root, the utilities
// below (w-(--sidebar-width) and friends) resolve without any inline style.
...
const SidebarInput = React.forwardRef<React.ElementRef<typeof Input>, React.ComponentProps<typeof Input>>(({ className, ...props }, ref) => {
    return (
        <Input
            ref={ref}
            data-sidebar="input"
            className={cn('h-8 w-full bg-surface shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring', className)}
            {...props}
        />
    );
});
    ...
    ({ className, asChild = false, ...props }, ref) => {
        ...
        return (
            <Comp
                ...
                className={cn(
                    'group-label flex h-8 shrink-0 items-center rounded-md px-2 outline-hidden ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
    // Inactive: muted text on a transparent row that tints on hover.
    // Active: brand tint background with brand-colored text and icon --
    // the treatment the reference design uses for the current page.
    'peer/menu-button flex w-full items-center gap-2.5 overflow-hidden rounded-nav px-2.5 py-2 text-left text-sm font-medium text-sidebar-foreground outline-hidden ring-sidebar-ring transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-semibold data-[active=true]:text-sidebar-accent-foreground data-[active=true]:hover:bg-primary-tint-hover data-[active=true]:hover:text-sidebar-accent-foreground data-[state=open]:hover:bg-muted group-data-[collapsible=icon]:mx-auto group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-subtle-foreground data-[active=true]:[&>svg]:text-sidebar-accent-foreground',
    {
        variants: {
            variant: {
                default: '',
                outline: 'bg-surface shadow-subtle hover:bg-muted',
            },
            size: {
                default: 'h-nav-item text-sm',
                sm: 'h-7 text-xs',
